...
{
    std::string fullPath = CCFileUtils::getInstance()->fullPathForFilename(iFilePath);
    this->confInfo_ = CCDictionary::createWithContentsOfFileThreadSafe(fullPath.c_str());
    if (this->confInfo_)
    {
    }
}